        ...
        <div className="flex min-w-0 flex-col gap-4 xl:col-span-2">
          {step === 0 ? (
            ...
            <SectionCard title={`Ítems · ${lines.length}`} contentClassName="p-0">
              <ul className="divide-y">{lines.map((l) => <li key={l.id} className="flex flex-wrap items-center gap-3 px-4 py-2.5 text-sm"><div className="min-w-0 flex-1"><p className="font-medium">{l.name}</p><p className="font-mono text-xs text-muted-foreground">{l.sku} · V.U. {formatCurrency(l.unitValue)}</p></div><label className="flex items-center gap-1 text-xs text-muted-foreground">Cant.<Input size="sm" value={l.qty} onChange={(e) => setLines((ls) => ls.map((x) => (x.id === l.id ? { ...x, qty: parseNumber(e.target.value) } : x)))} className="w-16 text-right font-mono" aria-label={`Cantidad ${l.sku}`} /></label><label className="flex items-center gap-1 text-xs text-muted-foreground">Desc. %<Input size="sm" value={l.discount} onChange={(e) => setLines((ls) => ls.map((x) => (x.id === l.id ? { ...x, discount: parseNumber(e.target.value) } : x)))} className="w-14 text-right font-mono" aria-label={`Descuento ${l.sku}`} /></label><span className="w-24 text-right font-mono font-medium">{formatCurrency(r2(l.qty * l.unitValue * (1 - l.discount / 100) * 1.18))}</span><Button size="icon-xs" variant="ghost" aria-label={`Quitar ${l.sku}`} onClick={() => setLines((ls) => ls.filter((x) => x.id !== l.id))}><Trash2 /></Button></li>)}{lines.length === 0 ? <li className="px-4 py-8 text-center text-sm text-muted-foreground">Agrega ítems del catálogo.</li> : null}</ul>
