                  ...
                  {lines.map((l, i) => {
                    ...
                    return (
                      <TableRow key={l.id}>
                        ...
                        <TableCell className="text-right">{ed ? <NumberInput size="sm" value={l.qty} onValueChange={(n) => update(l.id, { qty: n })} className="w-16 text-right font-mono" aria-label="Cantidad" /> : <span className="font-mono tabular-nums">{l.qty}</span>}</TableCell>
                        <TableCell>{l.unit}</TableCell>
                        <TableCell className="text-right">{ed ? <NumberInput size="sm" value={l.unitValue} onValueChange={(n) => update(l.id, { unitValue: n })} className="w-24 text-right font-mono" aria-label="Valor unitario" /> : <span className="font-mono tabular-nums">{l.unitValue.toFixed(2)}</span>}</TableCell>
                        <TableCell className="text-right">{ed ? <NumberInput size="sm" value={l.discountPct} onValueChange={(n) => update(l.id, { discountPct: n })} className="w-14 text-right font-mono" aria-label="Descuento" /> : <span className="font-mono tabular-nums">{l.discountPct}%</span>}</TableCell>
