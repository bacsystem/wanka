        ...
        <ul className="divide-y md:hidden">
          {lines.map((l) => { const d = l.counted - l.system; return (
            <li key={l.id} className="flex items-center gap-3 px-4 py-3">
              ...
              <NumberInput value={l.counted} onValueChange={(n) => setLines((ls) => ls.map((x) => (x.id === l.id ? { ...x, counted: n } : x)))} className="h-12 w-20 text-center font-mono text-lg" aria-label={`Conteo ${l.sku}`} />
            </li>
          ); })}
        </ul>
